...
__ii)__ For every token in every document, estimate \\( \hat{Pr}(w_i|S) = \frac{count(w_i|S)}{\Sigma_{w \in V} count(w_i|S)} \\), where \\( count(w_i|S) \\) is the number of times token  \\( w_i \\) appears __in all spam documents__, and  \\( \Sigma_{w \in V} count(w_i|S) \\) is the total number of words in all spam documents. Do a similar computation for Ham documents and tokens. 

__iii)__ Once we have these probabilities, we can compute \\( \hat{Pr}(S|w_1 , \cdots , w_D) \\) and \\( \hat{Pr}(H|w_1 , \cdots , w_D) \\) for any new document. Then we simply label that document as Spam or Ham, depending on which probability is larger.
